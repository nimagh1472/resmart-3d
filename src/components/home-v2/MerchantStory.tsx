...
const BEATS = ['Demand signal', 'Merchant activation', 'Sale confirmed.'];
...
/**
 * Homepage V2 — Merchant story. Same shape as ShopperStory.tsx (Phase 1),
 * intentionally duplicated rather than extracted into a shared component
 * so Phase 1's approved files stay untouched.
 */
export function MerchantStory() {
  const wrapperRef = useRef<HTMLDivElement>(null);
  const progress = useSpatialProgress(wrapperRef);
  const isDesktop = useIsDesktopViewport();
  const url = SPATIAL_ASSETS.merchant[isDesktop ? 'desktop' : 'mobile'];
      ...
      <section className="sticky top-0 flex h-screen w-full items-center justify-center overflow-hidden bg-[#050709]">
        ...
        <div className="relative z-10 flex flex-col items-center gap-8 px-6 text-center">
          <h2 style={{ fontFamily: 'ui-serif, Georgia, serif' }} className="text-2xl text-white sm:text-4xl">
            One signal reaches the right storefront.
          </h2>
